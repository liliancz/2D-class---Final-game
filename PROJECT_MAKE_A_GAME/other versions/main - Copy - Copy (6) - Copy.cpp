	...
	*/
	...
	for (int i = 0; i < size; i++)
	{
		...
		for (int j = i; j < size; j++)
		{
			...
			{
				countto4[i]++;
				//printf("Winner: %d  count: %d  \n", save_winner[i], countto4[i]);
				if (countto4[i] > 2)
				{
					
					number_winner = save_winner[i];
					printf("TEST WIN NUMbER %d \n", save_winner[i]);
	...
	//printf("winner number: %d \nwinner_array: ", number_winner);
	for (int i = 0; i < 12; i++)
	{
...
int main(int argc, char **argv)
{
	...
	bool pressed_bar = false;
	bool pressed_mousebotton = false;
	bool pressed_A = false;
	bool prev_pressed_mousebotton = false;
	bool prev_pressed_A = false;
	...
	while (!done)
	{
		...
		if (prev_pressed_mousebotton == false)pressed_mousebotton = SDL_GetMouseState(NULL, NULL);
		if (pressed_mousebotton & SDL_BUTTON(SDL_BUTTON_LEFT))
		{
			SDL_Log("SPIN");
			total_coins += cost_to_play;
			pressed_bar = true;
			prev_pressed_mousebotton = true;
		...
		if (prev_pressed_A == false ) pressed_A = keys[SDL_SCANCODE_A];

		if (pressed_A == true && prev_pressed_A == false)
		{
			printf("\n 1000 coins added\n");
			total_coins += 1000;
			print_coins(font_texture, total_coins);
		...
		while (pressed_bar)
		{
			...
			while (!SDL_TICKS_PASSED(SDL_GetTicks(), timeout))
			{
				...
				SDL_PumpEvents();
				/*
				if (SDL_BUTTON(SDL_BUTTON_LEFT) && prev_pressed_mousebotton == true)
				{
					SDL_Log("STOP PLEASE");
					prev_pressed_mousebotton = false;
					pressed_bar = false;
			...
			pressed_mousebotton = false;
			//prev_pressed_mousebotton = true;
		}

		prev_pressed_A = false;
		pressed_A = false;
		pressed_bar = false;
		prev_pressed_mousebotton = false;
